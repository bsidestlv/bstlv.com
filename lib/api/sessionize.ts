...
const BSIDESTLV_EVENT_ID =
  process.env.NEXT_PUBLIC_SESSIONIZE_EVENT_ID || "jl4ktls0"; // Fallback to demo
...
export class SessionizeAPI {
  ...
  async getSpeakers(): Promise<SessionizeSpeaker[]> {
    try {
      const response = await fetch(
        `${SESSIONIZE_BASE_URL}/${this.eventId}/view/speakers`,
        {
  ...
  async getSessions(): Promise<SessionizeSession[]> {
    try {
      const response = await fetch(
        `${SESSIONIZE_BASE_URL}/${this.eventId}/view/sessions`,
        {
          next: { revalidate: 3600 }, // Cache for 1 hour
        },
      );
  ...
  async getGrid(): Promise<SessionizeGrid[]> {
    try {
      const response = await fetch(
        `${SESSIONIZE_BASE_URL}/${this.eventId}/view/gridtable`,
        {
          next: { revalidate: 3600 }, // Cache for 1 hour
        },
      );
  ...
  async getRooms(): Promise<SessionizeRoom[]> {
    try {
      const response = await fetch(
        `${SESSIONIZE_BASE_URL}/${this.eventId}/view/rooms`,
        {
          next: { revalidate: 3600 }, // Cache for 1 hour
        },
      );
        ...
        sessions: [
          {
            ...
            description:
              "Detailed schedule will be published once the new event date is confirmed.",
            startsAt: "2025-06-26T08:30:00",
